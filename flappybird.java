...
public class flappybird extends JPanel implements ActionListener, KeyListener {
    ...
    public void move() {
        //bird
        if (!gameStarted) {
            return;  // Don't run game logic if it hasn't started
        }
        velocityY += gravity;
        bird.y += velocityY;
    ...
    public void keyPressed(KeyEvent e) {
        if (e.getKeyCode() == KeyEvent.VK_SPACE) {
            if (!gameStarted) {
                ...
                // If the game has started and is not over, make the bird jump
                velocityY = -6;
            } else {
                // If the game is over, pressing space will restart the game
                gameOver = false;
                score = 0;
                bird.y = birdY;
                pipes.clear();
